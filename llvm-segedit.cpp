//===-- llvm-segedit.cpp - Object file dumping utility for llvm -----------===//
//
...
extern cl::opt<bool> extractMachOSectionsToDisk;
...
static cl::opt<std::string> extractMachOSectionsToDiskDir(
    "extract-macho-sections-dir",
    cl::desc("Directory path to extract Mach-O sections to (requires -macho "
             "and -extract-macho-sections, extracts to current directory by "
             "default)"),
    cl::init("./"));

cl::opt<bool> llvm::extractMachOSectionsToDisk(
    "extract-macho-sections",
    cl::desc("Extract Mach-O sections to disk (requires -macho)"));
...
static const Target *GetTarget(const MachOObjectFile *MachOObj,
  ...
  *ThumbTarget = TargetRegistry::lookupTarget(ThumbTripleName, Error);
  ...
  WithColor::error(errs(), "llvm-segedit") << "unable to get target for '";
  if (!TheTarget)
    errs() << TripleName;
  else
      ...
      for (unsigned J = 0; J < Seg.nsects; ++J) {
        ...
        if (Sec.nreloc != 0) {
          ...
          if (O->getSectionName(DRI, SectName))
            ...
            outs() << "Relocation information (" << SegName << "," << SectName
                   << format(") %u entries", Sec.nreloc);
          outs() << "\naddress  pcrel length extern type    scattered "
                    "symbolnum/value\n";
...
static bool checkMachOAndArchFlags(ObjectFile *O, StringRef Filename) {
  ...
  if (none_of(ArchFlags,
    ...
    WithColor::error(errs(), "llvm-segedit")
...
static bool ExtractSectionData(std::string ifilename, std::string segSectName,
                               uint64_t Size, uint32_t Offset) {

  std::string ofilename = extractMachOSectionsToDiskDir;
  ofilename += llvm::sys::path::filename(SmallString<16>(ifilename));
  ofilename += "." + segSectName + ".bin";

  std::error_code EC;
  raw_fd_ostream ofile(ofilename.c_str(), EC, sys::fs::F_None);
  if (EC) {
    error(EC);
    return false;
  }

  auto buf = llvm::MemoryBuffer::getFile(ifilename);
  if (buf.getError()) {
    error(buf.getError());
    return false;
  }

  ofile.write(buf.get()->getBufferStart() + Offset, Size);
  if (ofile.has_error()) {
    errs() << "Error writing to file '" << ofilename << "'\n";
    return false;
  }
...
static bool ExtractSegmentCommand(StringRef filename,
                                  const MachOObjectFile &Obj,
                                  const MachOObjectFile::LoadCommandInfo &LCI) {
  for (unsigned i = 0; i != Obj.getSegmentLoadCommand(LCI).nsects; ++i) {
    auto Sect = Obj.getSection(LCI, i);
    std::string SectName(Sect.sectname, strnlen(Sect.sectname, 16));
    std::string SegName(Sect.segname, strnlen(Sect.segname, 16));
    std::string SectSegName = SectName + SegName;
    if (std::find(FilterSections.begin(), FilterSections.end(), SectName) ==
            FilterSections.end() &&
        FilterSections.size() > 0)
      continue;

    if (!ExtractSectionData(filename, SectSegName, Sect.size, Sect.offset))
      return false;
  }
  return true;
}
...
static bool
ExtractSegment64Command(std::string filename, const MachOObjectFile &Obj,
                        const MachOObjectFile::LoadCommandInfo &LCI) {
  for (unsigned i = 0; i != Obj.getSegment64LoadCommand(LCI).nsects; ++i) {
...
static bool ExtractLoadCommand(StringRef filename, const MachOObjectFile &Obj,
                               const MachOObjectFile::LoadCommandInfo &LCI) {
  if (LCI.C.cmd == MachO::LC_SEGMENT)
    return ExtractSegmentCommand(filename, Obj, LCI);
  else if (LCI.C.cmd == MachO::LC_SEGMENT_64)
    return ExtractSegment64Command(filename, Obj, LCI);
  return true;
}
...
void llvm::ParseInputMachO(StringRef Filename) {
  ...
  if (extractMachOSectionsToDisk) {
    const MachOObjectFile *InputObject = dyn_cast<MachOObjectFile>(&Bin);
    if (!InputObject) {
          ...
          if (ObjOrErr) {
            ...
            if (MachOObjectFile *MachOOF = dyn_cast<MachOObjectFile>(&O))
              ProcessMachO(Filename, MachOOF, "", ArchitectureName);
                         ...
                         ObjOrErr.takeError())) {
            report_error(Filename, StringRef(), std::move(E), ArchitectureName);
            continue;
...
static void DumpBitcodeSection(MachOObjectFile *O, const char *sect,
  ...
  std::error_code XarEC =
      sys::fs::createTemporaryFile("llvm-segedit", "xar", FD, XarFilename);
  if (XarEC) {
    WithColor::error(errs(), "llvm-segedit") << XarEC.message() << "\n";
    return;
  }
  ...
  std::error_code TocEC =
      sys::fs::createTemporaryFile("llvm-segedit", "toc", TocFilename);
  if (TocEC) {
    WithColor::error(errs(), "llvm-segedit") << TocEC.message() << "\n";
    return;
  }
  ...
  if (std::error_code EC = FileOrErr.getError()) {
    WithColor::error(errs(), "llvm-segedit") << EC.message() << "\n";
    return;
  }
      ...
      for (uint64_t Index = Start; Index < End; Index += Size) {
        ...
        if (gotInst) {
          ...
          if (Arch == Triple::x86_64 || Arch == Triple::x86) {
            ...
            WithColor::warning(errs(), "llvm-segedit")
...
cl::opt<bool> llvm::DynamicRelocations(
    "dynamic-reloc",
...
cl::list<std::string> llvm::MAttrs("mattr", cl::CommaSeparated,
...
static void DisassembleObject(const ObjectFile *Obj, bool InlineRelocs) {
  ...
  for (const SectionRef &Section : ToolSectionFilter(*Obj)) {
    ...
    if (Symbols.empty() || std::get<0>(Symbols[0]) != 0) {
      Symbols.insert(
          Symbols.begin(),
          std::make_tuple(SectionAddr, SectionName, 0));
    }
...
int main(int argc, char **argv) {
  ...
  if (!Disassemble && !Relocations && !DynamicRelocations && !SectionHeaders &&
      ...
      !FileHeaders && !FirstPrivateHeader && !ExportsTrie && !Rebase && !Bind &&
      !LazyBind && !WeakBind && !RawClangAST &&
      !(UniversalHeaders && MachOOpt) && !ArchiveHeaders &&
      !(IndirectSymbols && MachOOpt) && !(DataInCode && MachOOpt) &&
      !(LinkOptHints && MachOOpt) && !(InfoPlist && MachOOpt) &&
      !(DylibsUsed && MachOOpt) && !(DylibId && MachOOpt) &&
      !(ObjcMetaData && MachOOpt) &&
      !(extractMachOSectionsToDisk && MachOOpt) &&
